...
public abstract class Sniper extends Character implements CharacterInterface{
  protected int range;
  protected int arrows;
  protected int maxArrows;

  public Sniper(Names name, int hp, int maxHp, int damage, int defense, int initiative, int arrows, int maxArrows, int row, int col) {
  ...
  public void step(ArrayList<Character> teamFoe, ArrayList<Character> teamFriend) {
    ...
    for (Character c : getNotDeadTeamMembers(teamFriend)) {
      if (c == null) return;
      if (this.arrows < this.maxArrows && c.getClass() == Rustic.class && c.state.equals(States.READY)) {
        this.arrows += 1;
